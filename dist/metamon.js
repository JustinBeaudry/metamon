var Metamon = (function (assert,crypto$1,path,util) {
	...
	function enjoi(schema, options = {}) {
	  ...
	  function resolveAnyOf(current) {
	    assert$1(lodash.isArray(current.anyOf), 'Expected anyOf to be an array.');
	    return Joi.alternatives().try(current.anyOf.map(function (schema) {
	      return resolve(schema);
	    }));
	  }
	  function resolveAllOf(current) {
	    assert$1(lodash.isArray(current.allOf), 'Expected allOf to be an array.');
	  ...
	  function resolveOneOf(current) {
	    assert$1(lodash.isArray(current.oneOf), 'Expected allOf to be an array.');
	    return Joi.alternatives().try(current.oneOf.map(function (schema) {
	      return resolve(schema);
	    })).required();
	  }
	  function resolveNot(current) {
	    assert$1(lodash.isArray(current.not), 'Expected Not to be an array.');
