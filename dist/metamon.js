var Metamon = (function (assert,crypto$1,path,util) {
	'use strict';
	...
	internals.reachSet = function (obj, key, value) {
	    var path$$1 = key.split('.');
	    var ref = obj;
	    for (var i = 0; i < path$$1.length; ++i) {
	        var segment = path$$1[i];
	...
	exports.reach = function (obj, chain, options) {
	    ...
	    var path$$1 = chain.split(options.separator || '.');
	    var ref = obj;
	    for (var i = 0; i < path$$1.length; ++i) {
	        var key = path$$1[i];
	        ...
	        if (!ref || !(((typeof ref === 'undefined' ? 'undefined' : _typeof(ref)) === 'object' || typeof ref === 'function') && key in ref) || (typeof ref === 'undefined' ? 'undefined' : _typeof(ref)) !== 'object' && options.functions === false) {
	            exports.assert(!options.strict || i + 1 === path$$1.length, 'Missing segment', key, 'in reach path ', chain);
	...
	exports.transform = function (source, transform, options) {
	    ...
	    for (var _i8 = 0; _i8 < keys.length; ++_i8) {
	        var key = keys[_i8];
	        var path$$1 = key.split(separator);
	        var sourcePath = transform[key];
	        exports.assert(typeof sourcePath === 'string', 'All mappings must be "." delineated strings');
	        var segment = void 0;
	        var res = result;
	        while (path$$1.length > 1) {
	            segment = path$$1.shift();
	            if (!res[segment]) {
	                res[segment] = {};
	            }
	            res = res[segment];
	...
	internals.annotate = function (stripColorCodes) {
	    ...
	    for (var i = this.details.length - 1; i >= 0; --i) {
	        ...
	        for (var j = 0;; ++j) {
	            ...
	            if (j + 1 < path$$1.length && ref[seg] && typeof ref[seg] !== 'string') {
	                ref = ref[seg];
	            } else {
	...
	internals.root = function () {
	    ...
	    root.reach = function (schema, path$$1) {
	        Hoek.assert(schema && schema instanceof Any, 'you must provide a joi schema');
	        Hoek.assert(typeof path$$1 === 'string', 'path must be a string');
	        if (path$$1 === '') {
	            return schema;
	        }
	        ...
	        for (var i = 0; i < children.length; ++i) {
	            ...
	            if (child.key === key) {
	                return this.reach(child.schema, path$$1.substr(key.length + 1));
	            }
	        }
	    };
	...
	internals.Object = function (_Any) {
	    ...
	    _class.prototype.assert = function assert$$1(ref, schema, message) {
	        ref = Cast.ref(ref);
	        ...
	        return this._test('assert', { schema: schema, ref: ref }, function (value, state, options) {
	            ...
	            return this.createError('object.assert', { ref: path$$1, message: message }, localState, options);
	        });
	    };
	...
	internals.Array = function (_Any) {
	    ...
	    _class.prototype._checkItems = function _checkItems(items, wasArray, state, options) {
	        ...
	        for (var i = 0; i < il; ++i) {
	            ...
	            var path$$1 = wasArray ? state.path.concat(i) : state.path;
	            var localState = { key: key, path: path$$1, parent: state.parent, reference: state.reference };
	            var res = void 0;
	...
	exports.resolve = function() {
	  ...
	  for (var i = arguments.length - 1; i >= -1 && !resolvedAbsolute; i--) {
	    var path$$1 = (i >= 0) ? arguments[i] : process.cwd();
	    ...
	    resolvedAbsolute = path$$1.charAt(0) === '/';
	  }
	...
	exports.normalize = function(path$$1) {
	  var isAbsolute = exports.isAbsolute(path$$1),
	      trailingSlash = substr(path$$1, -1) === '/';
	  path$$1 = normalizeArray(filter(path$$1.split('/'), function(p) {
	    return !!p;
	  }), !isAbsolute).join('/');
	  if (!path$$1 && !isAbsolute) {
	    path$$1 = '.';
	  }
	  if (path$$1 && trailingSlash) {
	    path$$1 += '/';
	  }
	  return (isAbsolute ? '/' : '') + path$$1;
	};
	...
	exports.sep = '/';
	exports.delimiter = ':';
	exports.dirname = function(path$$1) {
	  var result = splitPath(path$$1),
	      root = result[0],
	      dir = result[1];
	  if (!root && !dir) {
	    return '.';
	...
	exports.basename = function(path$$1, ext) {
	  var f = splitPath(path$$1)[2];
	  if (ext && f.substr(-1 * ext.length) === ext) {
	    f = f.substr(0, f.length - ext.length);
	  }
	  return f;
	};
	exports.extname = function(path$$1) {
	  return splitPath(path$$1)[3];
	};
	...
	var lodash$2 = /*#__PURE__*/Object.freeze({
		default: lodash$1,
		__moduleExports: lodash$1
	});
	...
	exports.reach = function (obj, chain, options) {
	    ...
	    const path$$1 = chain.split(options.separator || '.');
	    let ref = obj;
	    for (let i = 0; i < path$$1.length; ++i) {
	        let key = path$$1[i];
	        if (key[0] === '-' && Array.isArray(ref)) {
	...
	exports.transform = function (source, transform, options) {
	    ...
	    for (let i = 0; i < keys.length; ++i) {
	        const key = keys[i];
	        const path$$1 = key.split(separator);
	...
	exports.uniqueFilename = function (path$$1, extension) {
	    ...
	    path$$1 = path.resolve(path$$1);
	    const name = [Date.now(), process.pid, crypto$1.randomBytes(8).toString('hex')].join('-') + extension;
	    return path.join(path$$1, name);
	};
	...
	var _ = ( lodash$2 && lodash$1 ) || lodash$2;

	var require$$0 = ( lib$1 && lib ) || lib$1;

	const {assert: assert$1} = require$$0;
	...
	function enjoi(schema, options = {}) {
	  ...
	  assert$1(!validateSchema.error, validateSchema.error);
	  const validateOptions = Joi.validate(options, optionsSchema);
	  assert$1(!validateOptions.error, validateOptions.error);
	  ...
	  function resolveref(value) {
	    ...
	    const path$$1 = value.substr(value.indexOf('#') + 1);
	    if (id && subSchemas) {
	    ...
	    assert$1(refschema, 'Can not find schema reference: ' + value + '.');
	  ...
	  function resolvetype(current) {
	    ...
	    function joitype(type, format) {
	      ...
	      assert$1(joischema, 'Could not resolve type: ' + current.type + '.');
	  ...
	  function resolveOneOf(current) {
	    assert$1(_.isArray(current.oneOf), 'Expected allOf to be an array.');
	    return Joi.alternatives().try(current.oneOf.map(function (schema) {
	      return resolve(schema);
	    })).required();
	  }
	  function resolveNot(current) {
	    assert$1(_.isArray(current.not), 'Expected Not to be an array.');
