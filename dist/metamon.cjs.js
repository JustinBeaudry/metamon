...
var rng$1 = /*#__PURE__*/Object.freeze({
	default: rng,
	__moduleExports: rng
});
...
var enjoi_min = createCommonjsModule(function (module) {
var Enjoi = (function () {
	...
	var Joi = unwrapExports$$1(joiBrowser);
	...
	Buffer$1.TYPED_ARRAY_SUPPORT = commonjsGlobal.TYPED_ARRAY_SUPPORT !== undefined
	...
	if (typeof commonjsGlobal.setTimeout === 'function') {
	    cachedSetTimeout = setTimeout;
	}
	if (typeof commonjsGlobal.clearTimeout === 'function') {
	    cachedClearTimeout = clearTimeout;
	}
	function runTimeout(fun) {
	    if (cachedSetTimeout === setTimeout) {
	...
	var schemaSchema = Joi.alternatives(Joi.object().unknown(true), Joi.string()).required();
	...
	function enjoi (schema) {
	    ...
	    var validateSchema = Joi.validate(schema, schemaSchema);
	    lib_20(!validateSchema.error, validateSchema.error);
	    var validateOptions = Joi.validate(options, optionsSchema);
	    lib_20(!validateOptions.error, validateOptions.error);
	    var _validateOptions$valu = validateOptions.value,
	        subSchemas = _validateOptions$valu.subSchemas,
	        types = _validateOptions$valu.types,
	        refineType = _validateOptions$valu.refineType,
	        strictMode = _validateOptions$valu.strictMode;
	    ...
	    function resolveref(value) {
	        var refschema = void 0;
	        var id = value.substr(0, value.indexOf('#') + 1);
	        var path = value.substr(value.indexOf('#') + 1);
	        if (id && subSchemas) {
	        ...
	        lib_20(refschema, 'Can not find schema reference: ' + value + '.');
	        var fragment = refschema;
	        var paths = path.split('/');
	        for (var i = 1; i < paths.length && fragment; i++) {
	            fragment = (typeof fragment === 'undefined' ? 'undefined' : _typeof(fragment)) === 'object' && fragment[paths[i]];
	        }
	        return fragment;
	    }
	    function resolvetype(current) {
	        var joischema = void 0;
	        ...
	        function joitype(type, format) {
	            ...
	            lib_20(joischema, 'Could not resolve type: ' + current.type + '.');
	            return joischema.strict(strictMode);
	        }
	        if (lodash.isArray(current.type)) {
	            var schemas = [];
	            for (var i = 0; i < current.type.length; i++) {
	    ...
	    function resolveAnyOf(current) {
	        lib_20(lodash.isArray(current.anyOf), 'Expected anyOf to be an array.');
	        return Joi.alternatives().try(current.anyOf.map(function (schema) {
	            return resolve(schema);
	        }));
	    }
	    function resolveAllOf(current) {
	        lib_20(lodash.isArray(current.allOf), 'Expected allOf to be an array.');
	        var type = getSchemaType(current.allOf[0]);
	        current.allOf.map(function (schema) {
	            var _type = getSchemaType(schema);
	            lib_20((_type === 'array' || _type === 'object') && type === _type, 'Expected allOf item to be an array or object.');
	        });
	        if (type === 'object') {
	    ...
	    function resolveOneOf(current) {
	        lib_20(lodash.isArray(current.oneOf), 'Expected allOf to be an array.');
	        return Joi.alternatives().try(current.oneOf.map(function (schema) {
	            return resolve(schema);
	        })).required();
	    }
	    function resolveNot(current) {
	        lib_20(lodash.isArray(current.not), 'Expected Not to be an array.');
	    ...
	    function resolveproperties(current) {
	        ...
	        Object.keys(current.properties).forEach(function (key) {
	            var property = current.properties[key];
	            var joischema = resolve(property);
...
var Enjoi = unwrapExports(enjoi_min);
