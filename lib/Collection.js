 ...
 */
class Collection extends _Serializable2.default {
  /**
   *
  ...
  constructor(data = {}, model, indexBy = 'id') {
    super();
    if (!model || !(model.prototype instanceof _Model2.default)) {
  ...
  set(data) {
    this.clear();
    switch ((0, _utils.getType)(data)) {
      case 'array':
        _lodash2.default.each(data, datum => setFromObject.call(this, datum));
        break;
      case 'object':
   ...
   */
  toArray() {
    return _lodash2.default.chain(this.index).values().map(item => convertToModel.call(this, item).toObject()).value();
  }
  /**
   *
   ...
   */
  toJSON() {
    return (0, _utils.toJSON)(this.toArray());
  }
}
...
function setFromObject(datum) {
  ...
  if (!key) {
    throw new _errors.MissingIndexError(model, this.indexBy);
  }
